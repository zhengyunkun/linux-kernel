...
void __init sched_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		root_task_group.real_policy = SCHED_NORMAL; // 全局调度器要采用CFS策略
...
// 增加对cgroup real_policy的读写操作
// 这两个函数允许通过 cgroup 接口读取和写入 real_policy 参数，从而动态调整任务组的调度策略。
static int cpu_real_policy_write_u64(struct cgroup_subsys_state *css,
                                struct cftype *cftype, u64 policy)
{
        struct task_group *tg = css_tg(css);
    tg->real_policy = policy;
        printk(KERN_ERR"cpu_real_policy_write_u64 : current.pid = %d, real_policy = %d, tg->css.id = %d\n", current->pid, tg->real_policy, tg->css.id);
    return 0;
}

static u64 cpu_real_policy_read_u64(struct cgroup_subsys_state *css,
                               struct cftype *cft)
{
        struct task_group *tg = css_tg(css);
        return (u64)tg->real_policy;
}
...
static struct cftype cpu_legacy_files[] = {
	...
	{
		.name = "real_policy",
		.read_u64 = cpu_real_policy_read_u64,
		.write_u64 = cpu_real_policy_write_u64,
	},
#endif
#ifdef CONFIG_CFS_BANDWIDTH
	{
